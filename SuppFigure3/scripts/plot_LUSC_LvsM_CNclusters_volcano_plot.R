...
patient_counts <- readRDS(file="data/NSCLCcohort/processed/patient_counts_compartment_3_CN_spatial_clusters_df.rds")

patient_counts <- patient_counts %>% filter(`Cancer Type`=="LUSC")

# the grouping variable
group_cols <- c("Patient", "MIBI clusters", "Compartment (3)")
...
output_folder <- "data/NSCLCcohort/output/Figure3/volcano_plots"
...
summary_df <- percentages_df %>%
    group_by_at(c("MIBI clusters", "Compartment (3)", "percentage")) %>%
    summarise(mean = mean(value)) %>%
    pivot_wider(names_from = all_of(c("MIBI clusters")), values_from = "mean") %>%
...
p.values <- percentages_df %>%
    ...
    group_modify(~{
        return(data.frame(p.value=wilcox.test(value~`MIBI clusters`, .x, exact=FALSE)$p.value))
    })
...
ggsave(file.path(output_folder, paste0("volcano_plot_LUSC_LvsM_CNclusters.png")))
